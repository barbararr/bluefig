...
import 'desktop_doctor_main_admin.dart';
import 'desktop_patient_main_admin.dart';

class DesktopSearchUserPageAdmin extends StatefulWidget {
  const DesktopSearchUserPageAdmin({Key? key}) : super(key: key);

  @override
  State<DesktopSearchUserPageAdmin> createState() =>
      _DesktopSearchUserPageAdminState();
}

class _DesktopSearchUserPageAdminState
    extends State<DesktopSearchUserPageAdmin> {
  ...
  void _getData() async {
    if (globals.searchInfo != '') {
      ...
      if (globals.userToSearch == "doctor") {
        patients = await ApiDataProvider()
            .serchPatient(globals.searchInfo, globals.doctorRoleId);
      } else {
        patients = await ApiDataProvider()
  ...
  void patientIdProfile(int index) {
    if (globals.userToSearch == "doctor") {
      globals.currentDoctorID = patients[index].id;
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Padding(
                  ...
                  child: Container(
                    child: globals.userToSearch == "doctor"
                        ? const Text(
                            'Доктора',
            ...
            patients.isEmpty
                ? const Text("пустой запрос или такого пользователя нет")
                : ListView.builder(
                    shrinkWrap: true,
